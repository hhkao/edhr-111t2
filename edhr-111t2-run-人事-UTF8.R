...
#私立中山工商(121405)
  #何曉富 吳俊德 唐美琪 姚譯婷 張瑟蘭 曾瑋 朱雅琳 李姈燕 李尚哲 林子軒 林文平 毛鳳敏 江瑜恒 涂玉嬿 王祺鑠 王麗玲 簡正佳 葉玲曲 蒲典聖 蔡寶月 蔡幸美 蘇漢章 許華欽 謝昀達 謝智鈞 郭媛婷 鍾曉慧 陳俐妤 陳家??? 陳理君 陳美月 陳菁徽 陳薇 黃宏仁 黃海山 黃熙達 黃琡雯 黃靖雯 黃顯貴   非上學期離退
...
check02$flag1 <- if_else(check02$flag1 == "尚待增補之學校主管：圖書館主管（請確認是否填報完整名單，倘貴校上開主任尚未到職，請來電告知）" & check02$organization_id == "121417", "", check02$flag1)
  #沒有設置學程主任
check02$flag3 <- if_else(check02$flag3 == "請學校確認是否設置科主任或學程主任" & check02$organization_id == "121417", "", check02$flag3)
  #放過學校 一年以上與任教領域相關之業界實務工作經驗人數偏多。
check02$flag18 <- if_else(check02$flag18 == "一年以上與任教領域相關之業界實務工作經驗人數偏多。（請再協助確認，『是否具備一年以上與任教領域相關之業界實務工作經驗』填寫『Y』之教員，是否確依欄位說明具備此經驗）" & check02$organization_id == "121417", "", check02$flag18)
...
check02$flag3 <- if_else(check02$flag3 == "請學校確認是否設置科主任或學程主任" & check02$organization_id == "181306", "", check02$flag3)
  #高中部教務主任、國中部教務主任
check02$flag18 <- if_else(check02$flag18 == "教務處主管人數超過一位，請再協助確認實際聘任情況。" & check02$organization_id == "181306", "", check02$flag18)
  #本項目不需請學校修正
...
check02$flag62 <- if_else(check02$flag62 == "教員資料表之行政職資料不完整或不正確：張瓊惠（兼任行政職服務單位(一)：技藝學程中心 兼任行政職職稱(一)：組長） 林正凰（兼任行政職服務單位(一)：圖書室 兼任行政職職稱(一)：組長） 翁韻茹（兼任行政職服務單位(一)：實習處即測即評及發證中心 兼任行政職職稱(一)：組長）（教員資料表之各兼任行政職資料不完整或不正確：請依欄位說明確認並正確填列行政單位名稱，如為二級單位主管，請敘明一級與二級單位名稱。如教務處教學組，學生事務處生活輔導組。）" & check02$organization_id == "201408", "", check02$flag62)
...
check02$flag86 <- if_else(check02$flag86 == "姓名：陳明堂（經比對貴校上一學年所填資料，上述人員並未出現於本學期的教員資料表或職員(工)資料表，請確認渠等是否於111學年度第一學期（111年8月1日-112年1月31日）退休或因故離職等，若於該學期退休或因故離職等，應於離退教職員(工)資料表填寫資料。如非於該學期退休或因故離職，或已介聘、調至他校，請來電告知。）" & check02$organization_id == "201408", "", check02$flag86)
  #本項目不需請學校修正
check02$flag95 <- if_else(check02$flag95 == "統計處專任教師人數：94人；本資料庫專任教師、代理教師、校長、教官、主任教官人數：100；差異百分比6.0%" & check02$organization_id == "201408", "", check02$flag95)
