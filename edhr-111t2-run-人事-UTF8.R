...
WHERE a.agree = 1 AND department_id = (SELECT id FROM [plat5_edhr].[dbo].[teacher_departments]
  ...
  subset(organization_id %in% c( #管區
    "421302",
    "381302",
    "331404",
    "421303",
...
path <- paste("\\\\192.168.110.244\\share_tmp\\給修恒\\1112\\edhr-111t2\\dta\\edhr_111t2-202303\\", title, ".xlsx", sep = "")
...
# 設定網址和檔名
url <- "https://stats.moe.gov.tw/files/detail/111/111_base2.xlsx"
filename <- "111_base2.xlsx"
...
for (x in try){
  data_schtype$try1[data_schtype$organization_id == x] <- 1
}
# 主管機關
...
flag_person$admin1 <- case_when(
  flag_person$sertype == "校長" | flag_person$admintitle1 == "校長" | flag_person$admintitle1 == "校長1" | flag_person$admintitle2 == "校長1" | flag_person$admintitle3 == "校長1" ~ 1,
...
flag_person_wide_flag18$err_admin5 <- if_else(flag_person_wide_flag18$count_admin5 > 1, "輔導室主管人數超過一位，請再協助確認實際聘任情況。", "")
...
flag_person$err_title0 <- if_else(!err_title0$admintitle0 %in% "N" & err_title0$admintitle0 %in% "組長" & ((!grepl("組$", err_title0$adminunit0) & !grepl("^N$", err_title0$adminunit0)) | (grepl("組$", err_title0$adminunit0) & nchar(err_title0$adminunit0) == 3)), 1, 0)

err_title1 <- data.frame(admintitle1 = flag_person$admintitle1, 
                         adminunit1 = flag_person$adminunit1, 
                         organization_id = flag_person$organization_id)
flag_person$err_title1 <- if_else(!err_title1$admintitle1 %in% "N" & err_title1$admintitle1 %in% "組長" & ((!grepl("組$", err_title1$adminunit1) & !grepl("^N$", err_title1$adminunit1)) | (grepl("組$", err_title1$adminunit1) & nchar(err_title1$adminunit1) == 3)), 1, 0)

err_title2 <- data.frame(admintitle2 = flag_person$admintitle2, 
                         adminunit2 = flag_person$adminunit2, 
                         organization_id = flag_person$organization_id)
flag_person$err_title2 <- if_else(!err_title2$admintitle2 %in% "N" & err_title2$admintitle2 %in% "組長" & ((!grepl("組$", err_title2$adminunit2) & !grepl("^N$", err_title2$adminunit2)) | (grepl("組$", err_title2$adminunit2) & nchar(err_title2$adminunit2) == 3)), 1, 0)

err_title3 <- data.frame(admintitle3 = flag_person$admintitle3, 
                         adminunit3 = flag_person$adminunit3, 
                         organization_id = flag_person$organization_id)
flag_person$err_title3 <- if_else(!err_title3$admintitle3 %in% "N" & err_title3$admintitle3 %in% "組長" & ((!grepl("組$", err_title3$adminunit3) & !grepl("^N$", err_title3$adminunit3)) | (grepl("組$", err_title3$adminunit3) & nchar(err_title3$adminunit3) == 3)), 1, 0)
...
flag_person$err_flag_2 <- if_else(flag_person$err_title0 != 0 | 
...
flag_person_wide_flag62_2_1$flag62_2_1_r <- gsub(" NA", replacement="", flag_person_wide_flag62_2_1$flag62_2_1_r)
flag_person_wide_flag62_2_1$flag62_2_1_r <- paste0(flag_person_wide_flag62_2_1$flag62_2_1_r, "\n（職員(工)資料表之各服務單位資料不完整或不正確：請依欄位說明確認並正確填列服務單位名稱，如為二級單位主管，請敘明一級與二級單位名稱。如總務處出納組，學生事務處生活輔導組。）") #若#err_flag_2_1: 職員工資料表出現err_flag_2，則加註
}else{
...
flag_person_wide_flag62_2_2$flag62_2_2_r <- gsub(" NA", replacement="", flag_person_wide_flag62_2_2$flag62_2_2_r)
flag_person_wide_flag62_2_2$flag62_2_2_r <- paste0(flag_person_wide_flag62_2_2$flag62_2_2_r, "\n（教員資料表之各兼任行政職資料不完整或不正確：請依欄位說明確認並正確填列行政單位名稱，如為二級單位主管，請敘明一級與二級單位名稱。如教務處教學組，學生事務處生活輔導組。）") #若err_flag_2_2: 教員資料表出現err_flag_2，則加註
}else{
...
flag_person_wide_flag62_2_3$flag62_2_3_r <- if_else(flag_person_wide_flag62_2_3$source == "職員(工)資料表", 
  paste0(flag_person_wide_flag62_2_3$flag62_2_3_r, "\n（教員資料表及職員(工)資料表之(兼任)職稱或服務單位資料不完整或不正確：請依欄位說明確認並正確填列行政單位名稱，如為二級單位主管，請敘明一級與二級單位名稱。如教務處教學組，總務處出納組。）"),  #若err_flag_2_3: 教員及職員工資料表同時出現err_flag_2，則加註
...
spe3 <- spe3 %>%
  ...
  mutate(spe3 = paste(spe3, "（請確認到職日期，並請以資料基準日112年3月31日當時情況為準）", sep = ""))
}else{
...
# 設定網址和檔名
url <- "https://stats.moe.gov.tw/files/detail/111/111_base0.xlsx"
filename <- "111_base0.xlsx"
...
colnames(moe_111_base0) <- moe_111_base0[3, ]
moe_111_base0 <- moe_111_base0[-c(1:4), ] %>%
...
if (dim(flag_person %>% subset(err_flag == 1))[1] != 0){
  ...
  flag96_temp <- flag_person_wide_flag96 %>%
    group_by(organization_id) %>%
    mutate(flag96_txt = paste("姓名：", flag96_r, sep = ""), "") %>%
...
#這裡是撈所有學校(drev_person_all)的資料來比對
...
#私立文德女中(401301)
  #確實沒有設置學務處主管 圖書館主管 人事室主管 主（會）計室主管(僅有組長)，沒有教務處主管 總務處主管 輔導室主管(將停招)
...
openxlsx :: write.xlsx(check02, file = "C:\\edhr-111t2\\work\\edhr-111t2-check_print-人事.xlsx", rowNames = FALSE, overwrite = TRUE)
}else{
openxlsx :: write.xlsx(check02, file = "C:\\edhr-111t2\\work\\edhr-111t2-check_print-人事.xlsx", rowNames = FALSE, overwrite = TRUE)
}
...
}else
{
  ...
  if(!file.exists("C:/edhr-111t2/dta/edhr_111t2-202303/pre_list_agree_人事.xlsx"))
  {
    ...
    openxlsx :: write.xlsx(pre_list_agree, file = "C:/edhr-111t2/dta/edhr_111t2-202303/pre_list_agree_人事.xlsx", rowNames = FALSE, overwrite = TRUE)
    #建立pre_correct_list.xlsx
    correct_list <- readxl :: read_excel("C:\\edhr-111t2\\work\\edhr-111t2-check_print-人事.xlsx") %>% #本次需補正學校
    ...
    openxlsx :: write.xlsx(correct_list, file = "C:/edhr-111t2/dta/edhr_111t2-202303/pre_correct_list_人事.xlsx", rowNames = FALSE, overwrite = TRUE)
  }else
  {
  ...
  {
    #讀取上次名單
    pre_list_agree <- readxl :: read_excel("C:/edhr-111t2/dta/edhr_111t2-202303/pre_list_agree_人事.xlsx")
    pre_list_agree$organization_id <- as.character(pre_list_agree$organization_id)
    pre_correct_list <- readxl :: read_excel("C:/edhr-111t2/dta/edhr_111t2-202303/pre_correct_list_人事.xlsx")
  ...
  where (substring(a.id, 4, 1) = '1' OR substring(a.id, 4, 1) = '2' OR substring(a.id, 4, 1) = '3' OR substring(a.id, 4, 1) = '4' OR substring(a.id, 4, 1) = 'B' OR substring(a.id, 4, 1) = 'C' OR substring(a.id, 4, 1) = 'F' OR substring(a.id, 4, 1) = 'G')")
    ...
    checkprint_filename <- "C:\\edhr-111t2\\work\\edhr-111t2-check_print-人事.xlsx"
    ...
    if(file.rename(from = checkprint_filename, to = paste(checkprint_filename_2, "2.xlsx", sep = "")) == TRUE)
    {
      ...
      }else
      {
        ...
        if(!file.exists(paste("C:\\autochecking\\",correct_filename, ".xlsx", sep = "")))
        {
          ...
          file.rename(from = "C:\\edhr-111t2\\work\\edhr-111t2-check_print-人事2.xlsx", to = "C:\\edhr-111t2\\work\\edhr-111t2-check_print-人事.xlsx")
        }
      }
    }else
    {
